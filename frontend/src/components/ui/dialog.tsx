...
const Dialog = ({ open, onOpenChange, children, size = 'md' }: DialogProps) => {
    if (!open) return null
    ...
    const dialogContent = (
        <div
            className="fixed inset-0 z-[10000] flex items-center justify-center animate-in fade-in duration-300 p-4"
        ...
        >
            ...
            <div
                className={cn(
                    "bg-slate-900/90 border border-white/10 text-white rounded-2xl shadow-2xl w-full relative animate-in zoom-in-95 duration-300 overflow-hidden z-10",
                    sizeClasses[size]
                )}
            ...
            >
                ...
                <button
                    onClick={() => onOpenChange && onOpenChange(false)}
                    className="absolute top-4 right-4 text-slate-500 hover:text-white transition-all p-1.5 rounded-xl hover:bg-purple-500/20 active:scale-90 z-10"
                    aria-label="Fechar"
                >
