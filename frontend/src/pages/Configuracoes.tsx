...
// --- Componente: Aba Email (SMTP) ---
function TabEmail({ config, setConfig, loadConfig }: {
    config: ConfiguracaoEmail
    setConfig: (c: ConfiguracaoEmail) => void
    ...
    useEffect(() => {
        if (user) setEmailTeste(user.email)
    }, [user])
        ...
        <div className="space-y-6 animate-in slide-in-from-right duration-500">
            ...
            <div className="forge-glass rounded-3xl border border-white/10 overflow-hidden">
                ...
                <div className="px-6 py-5 border-t border-white/5 bg-slate-800/30">
                    ...
                    {!config.smtp_configurado ? (
                        <div className="flex items-center gap-3 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-200 text-sm">
                            <div className="p-1.5 bg-amber-500/20 rounded-full">
                                <Shield className="w-4 h-4" />
                            </div>
                            <p>
                                Para realizar testes, preencha os campos acima (Host, Usuário, Senha) e clique em <strong>Salvar Configurações</strong>.
                            </p>
                        </div>
                    ) : (
...
// --- Componente: Aba Geral (Placeholder) ---
function TabGeral() {
    return (
        <div className="forge-glass rounded-3xl border border-white/10 p-12 text-center animate-in slide-in-from-right duration-500">
            <div className="w-20 h-20 bg-slate-800/50 rounded-full flex items-center justify-center mx-auto mb-6">
                <LayoutGrid className="w-10 h-10 text-slate-500" />
            </div>
            <h2 className="text-xl font-semibold text-white mb-2">Configurações Gerais</h2>
            <p className="text-slate-400 max-w-md mx-auto">
                Em breve você poderá configurar preferências gerais da empresa, como logo, cores e políticas de acesso.
            </p>
        </div>
    )
}
...
export default function Configuracoes() {
    ...
    const [activeTab, setActiveTab] = useState<'geral' | 'email'>('email')
                ...
                <div className="flex flex-col lg:flex-row gap-8 flex-1 min-h-0">
                    ...
                    <div className="w-full lg:w-64 flex-shrink-0 space-y-2">
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${activeTab === 'email'
                                ? 'bg-purple-500/20 text-white shadow-lg shadow-purple-500/10 border border-purple-500/30'
                                : 'text-slate-400 hover:bg-white/5 hover:text-white'
                                }`}
                        >
                            <Mail className={`w-5 h-5 ${activeTab === 'email' ? 'text-purple-400' : ''}`} />
                            <span className="font-medium">Email (SMTP)</span>
                        </button>

                        <button
                            onClick={() => setActiveTab('geral')}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left ${activeTab === 'geral'
                                ? 'bg-purple-500/20 text-white shadow-lg shadow-purple-500/10 border border-purple-500/30'
                                : 'text-slate-400 hover:bg-white/5 hover:text-white'
                                }`}
                        >
                            <LayoutGrid className={`w-5 h-5 ${activeTab === 'geral' ? 'text-purple-400' : ''}`} />
                            <span className="font-medium">Geral</span>
                        </button>

                        <button
                            disabled
                            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all text-left text-slate-600 cursor-not-allowed opacity-50"
                        >
                            <Webhook className="w-5 h-5" />
                            <div className="flex-1">
                                <span className="font-medium block">Integrações</span>
